...
def weighted_loss(loss: dict, weight, prefix="unsup", ignore_keys=[], warmup=0):
    for name, loss_item in loss.items():
        if name in ignore_keys:
            continue
        if ('loss' in name) and (prefix in name):
            # print(loss_item)
            # print(name, loss_item)
            # if isinstance(loss_item, Sequence):
            #     loss[name] = [item * weight for item in loss_item]
            # elif isinstance(loss_item, Number):
            #     loss[name]
            loss[name] = sequence_mul(loss_item, weight)
    return loss


def weighted_all_loss(loss:dict, weight):
    for name, loss_item in loss.items():
        if 'loss' in name:
            loss[name] = sequence_mul(loss_item, weight)
    return loss
