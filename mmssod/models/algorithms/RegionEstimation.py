...
class RegionEstimation(BurnInTSModel):
    ...
    def _gen_pseudo_labels(self, unsup_data):
        ...
        if self.print_pesudo_summary:
            self._add_summary_bboxes([{"bboxes":torch.cat([bboxes.clone(), torch.unsqueeze(scores.clone(), dim=1)], dim=1).detach().cpu().numpy(), "labels":labels.clone().detach().cpu().numpy()} for (bboxes, scores, labels) in zip(gt_bboxes, gt_scores, gt_labels)],
