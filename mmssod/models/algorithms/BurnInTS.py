...
class BurnInTSModel(BaseDetector):
    ...
    def __init__(self, teacher: dict, student: dict, train_cfg=None, test_cfg=None, n_cls=20):
        ...
        self.unreliable_det_bboxes_list = []
        self.recall_pros = []
        ...
        self.neg_unreliable_strong_sim_list = []
        self.neg_unreliable_weak_sim_list = []
        self.neg_unreliable_bboxes_scores_list = []

        self.analysis_recall_unreliable_bboxes=[]
        self.analysis_recall_unreliable_scores=[]
        self.analysis_recall_unreliable_labels=[]
        self.analysis_recall_true_bboxes=[]
        self.analysis_recall_true_labels=[]
        self.analysis_recall_iou_preds = []
        self.analysis_recall_sim=[]
    ...
    def _add_recall_pro(self, recall_pro):
        self.recall_pros.append(recall_pro)
    ...
    def _add_unreliable_bboxes(self, unreliable_results):
        self.unreliable_det_bboxes_list.append(unreliable_results)
    ...
    def _add_neg_loss_info(self,
                           unreliable_mask,
                           unreliable_bboxes,
                           unreliable_bboxes_scores,
                           strong_sim=None,
                           weak_sim=None):
        self.neg_unreliable_mask_list.append(unreliable_mask)
        self.neg_unreliable_bboxes_list.append(unreliable_bboxes)
        self.neg_unreliable_bboxes_scores_list.append(unreliable_bboxes_scores)
        self.neg_unreliable_strong_sim_list.append(strong_sim)
        self.neg_unreliable_weak_sim_list.append(weak_sim)

    # def _add_informativenee_info(self,
    #                              scores,
    #                              entropys,
    #                              rois):

    def _add_iou_info(self,
                      rois,
                      roi_weights):
        self.iou_roi_list.append(rois)
        self.iou_pred_list.append(roi_weights)

    def _add_analysis_recall_info(self,
                                  unreliable_bboxes,
                                  unreliable_scores,
                                  unreliable_labels,
                                  true_bboxes,
                                  ...
                                  iou_preds,
                                  sim=None):
        self.analysis_recall_unreliable_bboxes.append(unreliable_bboxes)
        self.analysis_recall_unreliable_labels.append(unreliable_labels)
        self.analysis_recall_unreliable_scores.append(unreliable_scores)
        self.analysis_recall_true_bboxes.append(true_bboxes)
        self.analysis_recall_true_labels.append(true_labels)
        self.analysis_recall_iou_preds.append(iou_preds)
        self.analysis_recall_sim.append(sim)
    ...
    def log_recall_precisions(self, epoch_num=0, iter_num=0):
        ...
        with open(info_path, 'wb') as f:
            ...
            pickle.dump({"unraliable_bboxes": self.neg_unreliable_bboxes_list,
                         ...
                         "unreliable_bboxes_scores": self.neg_unreliable_bboxes_scores_list,
                         "gts_bboxes": self.measure_gts_list,
                         "strong_sim": self.neg_unreliable_strong_sim_list,
                         "weak_sim": self.neg_unreliable_weak_sim_list
                         }, f)

        summary_path = osp.join(work_dir, 'pred_iou_infos')
        mmcv.mkdir_or_exist(osp.abspath(summary_path))
        info_path = osp.join(summary_path, 'iter_%d_rank_%d.pkl' % (iter_num, rank))
        with open(info_path, 'wb') as f:
            # if len(self.pos_roi_weight_list) != 0:
            pickle.dump({"iou_rois": self.iou_roi_list,
                         "iou_preds": self.iou_pred_list,
                         "gts_bboxes": self.measure_gts_list}, f)

        summary_path = osp.join(work_dir, 'unreliable_bboxes')
        mmcv.mkdir_or_exist(osp.abspath(summary_path))
        info_path = osp.join(summary_path, 'iter_%d_rank_%d.pkl' % (iter_num, rank))
        with open(info_path, 'wb') as f:
            # if len(self.pos_roi_weight_list) != 0:
            pickle.dump({"unraliable_bboxes": self.unreliable_det_bboxes_list
                         }, f)

        summary_path = osp.join(work_dir, 'recall_pros')
        mmcv.mkdir_or_exist(osp.abspath(summary_path))
        info_path = osp.join(summary_path, 'iter_%d_rank_%d.pkl' % (iter_num, rank))
        with open(info_path, 'wb') as f:
            # if len(self.pos_roi_weight_list) != 0:
            pickle.dump({"recall_pros": self.recall_pros
                         }, f)

        summary_path = osp.join(work_dir, 'analysis_recall')
        mmcv.mkdir_or_exist(osp.abspath(summary_path))
        info_path = osp.join(summary_path, 'iter_%d_rank_%d.pkl' % (iter_num, rank))
        with open(info_path, 'wb') as f:
            # if len(self.pos_roi_weight_list) != 0:
            pickle.dump({"analysis_recall_unreliable_bboxes": self.analysis_recall_unreliable_bboxes,
                         "analysis_recall_unreliable_scores": self.analysis_recall_unreliable_scores,
                         "analysis_recall_unreliable_labels": self.analysis_recall_unreliable_labels,
                         "analysis_recall_true_bboxes": self.analysis_recall_true_bboxes,
                         "analysis_recall_true_labels": self.analysis_recall_true_labels,
                         "analysis_recall_iou_pred": self.analysis_recall_iou_preds,
                         "analysis_recall_sim": self.analysis_recall_sim}, f)
