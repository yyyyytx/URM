...
class NegativeLoss(BurnInTSModel):
    def __init__(self, teacher: dict, student: dict, train_cfg=None, test_cfg=None, n_cls=20):
        ...
        self.is_soft_label_filter = train_cfg.get(
            "is_soft_label_filter", False)
    ...
    def forward_train(self, imgs, img_metas, **kwargs):
        ...
        strong_unsup, weak_unsup = self._gen_unreliable_pseudo_labels(
            weak_unsup, strong_unsup)

        # self._accumulate_train_number_per_category(strong_unsup['gt_labels'])

        unsup_loss = self._compute_student_unsup_negative_loss(
            weak_unsup, strong_unsup)
    ...
    def _analysis_recall_sim_thr(self, img_feat, unreliable_bboxes, unreliable_scores, unreliable_labels, true_bboxes, true_labels):
        ...
        for i in range(len(unreliable_bboxes)):
            ...
            rois = torch.cat(
                [unreliable_bboxes[i], unreliable_scores[i].reshape(-1, 1)], dim=1)
        ...
        iou_pred = self.teacher.roi_head.extract_bboxes_features(
            img_feat, roi_list)
    ...
    def _recall_rois(self, img_feat, unreliable_bboxes, unreliable_scores, unreliable_labels, true_labels=None):
        ...
        for labels, scores in zip(unreliable_labels, unreliable_scores):
            ...
            mask = torch.rand(len(sample_pro)).to(
                sample_pro.device) < sample_pro
        ...
        bboxes = [recall_bboxes[mask]
                  for (recall_bboxes, mask) in zip(unreliable_bboxes, recall_masks2)]
        labels = [recall_labels[mask]
                  for (recall_labels, mask) in zip(unreliable_labels, recall_masks2)]
        scores = [recall_scores[mask]
                  for (recall_scores, mask) in zip(unreliable_scores, recall_masks2)]
    ...
    def parse_informativeness(self, weak_unsup):
        ...
        with torch.no_grad():
            ...
            proposal_list = self.teacher.rpn_head.simple_test_rpn(
                x, weak_unsup['img_metas'])
            ...
            img_shapes = tuple(meta['img_shape']
                               for meta in weak_unsup['img_metas'])
    ...
    def _gen_unreliable_pseudo_labels(self, weak_unsup, strong_unsup):
        ...
        if self.print_pesudo_summary:
            ...
            strong_unsup.update(
                {"gt_bboxes_true": [bboxes for bboxes in strong_unsup["gt_bboxes"]]})
            strong_unsup.update(
                {"gt_labels_true": [labels for labels in strong_unsup["gt_labels"]]})
        else:
        ...
        reliable_bboxes = [bboxes[scores >= self.pesudo_thr]
                           for (bboxes, scores) in zip(result_bboxes, result_scores)]
        reliable_labels = [labels[scores >= self.pesudo_thr]
                           for (labels, scores) in zip(det_labels, result_scores)]
        reliable_scores = [scores1[scores2 >= self.pesudo_thr]
                           for (scores1, scores2) in zip(result_scores, result_scores)]
        ...
        if self.print_pesudo_summary:
            strong_unsup.update(
                {"reliable_bboxes": [bboxes.cpu().clone() for bboxes in reliable_bboxes]})
            strong_unsup.update(
                {"reliable_labels": [labels.cpu().clone() for labels in reliable_labels]})

            strong_unsup.update(
                {"recall_bboxes": [bboxes.cpu().clone() for bboxes in unreliable_bboxes]})
            strong_unsup.update(
                {"recall_labels": [labels.cpu().clone() for labels in unreliable_labels]})
        ...
        if self.is_recall is True:
            ...
            recall_bboxes = [bboxes.clone()
                             for bboxes in unreliable_bboxes]

            recall_labels = [labels.clone()
                             for labels in unreliable_labels]
            recall_scores = [scores.clone()
                             for scores in unreliable_scores]
            ...
            filter_mask, mix_pro_list, t_score_list, s_score_list = self._t_s_filter_bboxes(
                feat, s_feat, recall_bboxes)
            if self.print_pesudo_summary:
                _, mix_reliable_pro_list, t_reliable_score_list, s_reliable_score_list = self._t_s_filter_bboxes(
                    feat, s_feat, reliable_bboxes)
            recall_bboxes = [bboxes[mask]
                             for (bboxes, mask) in zip(recall_bboxes, filter_mask)]
            recall_labels = [labels[mask].type(torch.long) for (
                labels, mask) in zip(recall_labels, filter_mask)]
            recall_scores = [scores[mask]
                             for (scores, mask) in zip(recall_scores, filter_mask)]
            ...
            unreliable_bboxes = [bboxes[~mask] for (
                bboxes, mask) in zip(unreliable_bboxes, filter_mask)]
            unreliable_labels = [labels[~mask].type(torch.long) for (
                labels, mask) in zip(unreliable_labels, filter_mask)]
            unreliable_scores = [scores[~mask] for (
                scores, mask) in zip(unreliable_scores, filter_mask)]
            unreliabel_mix_pro = [pro[~mask]
                                  for (pro, mask) in zip(mix_pro_list, filter_mask)]
            ...
            _, mix_pro_list, t_score_list, s_score_list = self._t_s_filter_bboxes(
                feat, s_feat, unreliable_bboxes)
        ...
        if self.print_pesudo_summary:
            self._analysis_recall_sim_thr(feat, result_bboxes, result_scores, det_labels, [bboxes.cpu().clone(
            ) for bboxes in weak_unsup["gt_bboxes"]], [labels.cpu().clone() for labels in weak_unsup["gt_labels"]])
        ...
        if self.print_pesudo_summary:
            if self.is_recall:
                self._add_summary_bboxes([{"bboxes": torch.cat([bboxes, torch.unsqueeze(scores, dim=1)], dim=1).detach().cpu().numpy(),
                                           ...
                                           "all_bboxes": all_bbox.detach().cpu().numpy(),
                                           "all_labels": all_label.detach().cpu().numpy(),
                                           } for (bboxes, labels, scores, r_bboxes, r_labels, t_soft_label, s_soft_label, mask,
                                                  re_bboxes, re_labels, t_re_soft_label, s_re_soft_label, gts_bboxes, gts_labels, all_bbox, all_label) in
                                          zip(strong_unsup["gt_bboxes"], strong_unsup["gt_labels"], strong_unsup["gt_scores"],
                                              strong_unsup["recall_bboxes"], strong_unsup[
                                              "recall_labels"], t_score_list, s_score_list, filter_mask,
                                              strong_unsup["reliable_bboxes"], strong_unsup[
                                              "reliable_labels"], t_reliable_score_list, s_reliable_score_list,
                                              weak_unsup["gt_bboxes"], weak_unsup["gt_labels"], det_bboxes, det_labels)],
    ...
    def _t_s_filter_bboxes(self,  t_feat, s_feat, gt_bboxes):
        ...
        for i in range(len(num_proposals_per_img)):
            t_score_list.append(
                t_cls_score[ind:ind + num_proposals_per_img[i]])
            ind += num_proposals_per_img[i]
        ...
        for i in range(len(num_proposals_per_img)):
            s_score_list.append(
                s_cls_score[ind:ind + num_proposals_per_img[i]])
            ind += num_proposals_per_img[i]
    ...
    def _nms_bboxes(self, gt_bboxes, gt_labels, gt_scores):
        ...
        for i in range(len(gt_bboxes)):
            ...
            scores = gt_scores[i]
            labels = gt_labels[i]
            nms_scores = torch.zeros(
                (len(gt_bboxes[i])), self.n_cls + 1).to(bboxes.device)
    ...
    def _compute_student_unsup_negative_loss(self, weak_unsup, strong_unsup):
        ...
        roi_losses = self._compute_student_unsup_losses(
            weak_unsup, strong_unsup)
        ...
        if self.is_neg_loss is True:
            neg_loss, roi_masks, bboxes_scores, grad1, grad2 = self.student.roi_head._unreliable_neg_loss(x=strong_feat,
                                                                                                          bboxes=strong_unsup[
                                                                                                              "unreliable_bboxes"],
                                                                                                          bboxes_scores=strong_unsup[
                                                                                                              "unreliable_mix_pro"],
                                                                                                          n_cls=self.n_cls,
                                                                                                          top_thr=self.topk,
                                                                                                          thr=self.neg_center)
            ...
            if self.print_pesudo_summary:
                self._add_neg_loss_info(roi_masks,
                                        [bboxes.detach().cpu().numpy()
                                         for bboxes in strong_unsup["unreliable_bboxes"]],
                                        {'grad1': grad1,
                                         'grad2': grad2})
    ...
    def _compute_student_unsup_losses(self, weak_unsup, strong_unsup, proposals=None):
        ...
        if self.is_ignore_ubreliable is True:
            ignore_bboxes = self._cal_ignore_unreliable_bboxes(
                strong_unsup["gt_bboxes"], strong_unsup["unreliable_bboxes"])
        else:
            ignore_bboxes = None
    ...
    def _cal_ignore_unreliable_bboxes(self, gt_bboxes, unreliable_bboxes):
        ...
        result_bbox = [unralia[mask]
                       for unralia, mask in zip(unreliable_bboxes, masks)]
        return result_bbox
