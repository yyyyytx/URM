...
class Transform2D:
    ...
    def transform_bboxes(bbox, M, out_shape):
        if isinstance(bbox, Sequence):
            ...
            return [
                Transform2D.transform_bboxes(b, m.to(b.device), o)
...
def mean_bboxes(bboxes):
    # if isinstance(bboxes, list):
    #     mean_bboxes_list = []
        # for i in range(len(bboxes)):
    if bboxes.shape[1] == 4:
        mean_bboxes = torch.mean(bboxes, dim=0)
    elif bboxes.shape[1] == 5:
        # [tl_x, tl_y, br_x, br_y, score]
        mean_bboxes = bboxes[:, :4]
        mean_bboxes = torch.mean(mean_bboxes, dim=0)
    return mean_bboxes.reshape((1, -1))

# if __name__ == '__main__':
#     aa = torch.rand((2, 4))
#     print(aa)
#     print(mean_bboxes(aa))
#
