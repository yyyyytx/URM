...
class DistributedSemiBatchSampler(Sampler):
    ...
    def _gen_indices_of_label(self):
        self.gen_flag += 1
        g = torch.Generator()
        g.manual_seed(self.epoch + self.seed + self.gen_flag)
...
class DistributedInfiniteSemiGroupBatchSampler(Sampler):
    def __init__(self,
                 dataset,
                 sample_ratio=None,
                 samples_per_gpu=1,
        ...
        self.epoch_length = len(self.indice_batches)
        # random.shuffle(self.indice_batches)

    def _gen_semi_batch(self):
        self.gen_flag += 1
        np.random.seed(self.seed + self.epoch + self.gen_flag)
        indice_batches = []
    ...
    def __iter__(self):
        ...
        random.shuffle(self.indice_batches)
        self.epoch_length = len(self.indice_batches)
        self.indice_batches = iter(self.indice_batches)
        while True:
            try:
                batch_item = next(self.indice_batches)
            except:
                print('re-gen label index')
                self.indice_batches = self._gen_semi_batch()
                self.indice_batches = iter(self.indice_batches)
