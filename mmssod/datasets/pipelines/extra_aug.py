...
# _EXCLUDED_TRANSFORMS = ['RandomGrayscale', 'ColorJitter', 'RandomErasing']
# for m in inspect.getmembers(_transforms, inspect.isclass):
#     if m[0] in _EXCLUDED_TRANSFORMS:
...
class RandColorJitter(object):
    def __init__(self, brightness=0, contrast=0, saturation=0, hue=0, p=0.5):
        self.trans = _transforms.ColorJitter(brightness,
                                             contrast,
                                             saturation,
                                             hue)
        self.prob = p
    ...
    def __call__(self, results):
        if np.random.random() < self.prob:
            for key in results.get("img_fields", ["img"]):
                img = results[key]
                results[key] = np.asarray(self.trans(Image.fromarray(img.astype(np.uint8))),
                                          dtype=img.dtype)
...
class RandGaussianBlur(object):
    def __init__(self, sigma_min=0.1, sigma_max=2.0, p=0.5):
        self.trans = GaussianBlur(sigma_min, sigma_max, p)
...
class RandErasing(object):
    def __init__(self, p, scale, ratio, value="random"):
        self.trans = _transforms.RandomErasing(p, scale, ratio, value)

    def __call__(self, results):
        for key in results.get("img_fields", ["img"]):
            img = results[key]
            results[key] = np.asarray(self.trans(torch.from_numpy(img.transpose((2, 0, 1))).contiguous()),
                                      dtype=img.dtype).transpose((1, 2, 0))
        return results
...
class MultiBranch(object):
    ...
    def __call__(self, results):
        multi_results = []
        multi_results.append(self.transform_group['weak'](copy.deepcopy(results)))
        multi_results.append(self.transform_group['strong'](copy.deepcopy(results)))
